\begin{frame}
  ...
  Multiplying \cref{eq:even-parity-equation} by $v$, integrating over an angular and spatial element gives, and applying integration by parts gives the weak form:
  \begin{multline}
    ...
    \intelems\sigt\psiplus\elemdifs=\intelems vS(x,y)\elemdifs
  \end{multline}
...
\begin{frame}
  ...
  \begin{multline}\label{eq:weak-form}
    -\frac{1}{\sigt}\int_{\mathcal{A}}\oint_{\partial\tau}v\left(\mu\pdv{\psiplus}{x}+\eta\pdv{\psiplus}{y}\right)\ohat\cdot\hat{n}\dd s\dd\ohat +\\+\intelems\left(\mu\pdv{\psiplus}{x}+\eta\pdv{\psiplus}{y}\right)\left(\mu\pdv{v}{x}+\eta\pdv{v}{y}\right)\elemdifs+\\
    +\sigt\intelems v \psiplus\elemdifs=\intelems vS(x,y)\elemdifs
  \end{multline}
\end{frame}
...
\begin{frame}
  ...
  \begin{equation}\label{eq:galerkin-test-function}
    v(x,y,\theta,\vartheta)=\sum_m\sum_n T_m(x,y)A_n(\theta,\varphi)
  \end{equation}
