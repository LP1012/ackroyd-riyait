\begin{frame}
  ...
  \begin{subequations}
    \begin{equation}
      ...
      \left.\dv{\psi}{x}\right|_{i,j} = \frac{\psi^{i+1/2}-\psi^i}{h}+\mathcal{O}\left(h\right)
    \end{equation}
    \begin{equation}
      \label{eq:backward-diff}
      \left.\dv{\psi}{x} \right|_{i,j} = \frac{\psi^{i}-\psi^{i-1/2}}{h}+\mathcal{O}\left(h\right)
    \end{equation}
  \end{subequations}
  ...
  \end{equation}
  \cref{eq:diamond-differencing} is known as the \textit{diamond differencing} closure~\cite{lewisComputationalMethodsNeutron1993} and is 2\textsuperscript{nd}-order accurate in space.
