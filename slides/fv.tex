\begin{frame}[allowframebreaks]
  ...
  \begin{multline}\label{eq:fv-eqs}
    \frac{\mu_{m}}{h_x}\left(\psi_{mn}^{i+1/2,j}-\psi_{mn}^{i-1/2,j}\right)+\frac{\eta_{n}}{h_y}\left(\psi_{mn}^{i,j+1/2}\-\psi_{mn}^{i,j-1/2}\right) + \Sigma_t ^{ij}\psi_{mn}^{ij} =\\=\frac{\Sigma_s^{ij}}{4\pi}\phi^{ij} + \frac{Q^{ij}\left(\rvec\right)}{4\pi}
  \end{multline}
