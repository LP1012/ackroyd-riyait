\begin{frame}[allowframebreaks]
  ...
  \begin{multline}\label{eq:fv-eqs}
    \mu_{m}\left(\psi_{mn}^{i+1/2,j}-\psi_{mn}^{i-1/2,j}\right)+\eta_{n}\left(\psi_{mn}^{i,j+1/2}\-\psi_{mn}^{i,j-1/2}\right) + \Sigma_t ^{ij}\psi_{mn}^{ij} =\\=\frac{\Sigma_s^{ij}}{4\pi}\phi^{ij} + \frac{Q^{ij}\left(\rvec\right)}{4\pi}
  \end{multline}
